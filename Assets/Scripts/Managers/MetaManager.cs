...
public class MetaManager : MonoBehaviour
{
                ...
                for (int index = 0; index < metaInfoData.arrayOfMetaData.Length; index++)
                {
                    ...
                    if (tempList.Count == 0)
                    {
                        ...
                        MetaData metaData = new MetaData()
                        {
                            ...
                            spriteName = GameManager.i.spriteScript.infoSprite.name,
                            isActive = false,
                            isRecommended = false,
                            isSelected = false,
                            isCriteria = false,
    ...
    private void InitialiseHQ()
    {
        ...
        Debug.AssertFormat(transitionInfoData.listOfWorkerArcs.Count <= GameManager.i.hqScript.maxNumOfWorkers, "Mismatch on listOfWorkerNames (has {0} records, should be {1}",
